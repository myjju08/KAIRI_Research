...
def process_deepcache_merged_experiments(experiment_base_dir, experiment_name, target_class):
    """Process merged DeepCache experiments"""
    ...
    # merged 폴더에서는 바로 cache_X_clean_Y 폴더들을 찾음
    for item in os.listdir(experiment_base_dir):
        if item.startswith("cache_") and os.path.isdir(os.path.join(experiment_base_dir, item)):
            ...
            try:
                ...
                accuracy = calculate_condition_accuracy_for_deepcache_experiment(cache_clean_dir, target_class)
...
def main():
    parser = argparse.ArgumentParser(description='Calculate condition accuracy for merged DeepCache experiments')
    parser.add_argument('--experiment_dir', type=str, required=True, 
                       help='Merged experiment directory')
    parser.add_argument('--target_class', type=int, required=True,
                       help='Target class for condition accuracy calculation')
    ...
    # DeepCache 실험 처리
    results = process_deepcache_merged_experiments(args.experiment_dir, experiment_name, args.target_class)
