...
class AduioInpaintingGuidance:
    ...
    def _log_ref(self):
        ...
        x_length = self.images.shape[-1]
        mask[:, :, :, x_length // 2 - 40 : x_length // 2 + 40] = 0
        ...
        ref_audios = decode(self.images) * 30
    ...
    def get_guidance(self, x_need_grad, func=lambda x: x, post_process=lambda x:x, return_logp=False, check_grad=True, **kwargs):
        ...
        # mask 40 frames in the middle
        mask[:, :, :, x_length // 2 - 20 : x_length // 2 + 20] = 0
